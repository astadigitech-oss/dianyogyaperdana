<div>
    <button wire:click='toggle' href=""
        class="w-full block shadow-xl hover:-translate-y-[10px] hover:cursor-pointer duration-500 transition-all h-[250px] rounded-[8px] overflow-hidden">
        <img src="{{ asset('storage/' . $src) }}" class="w-full h-full object-cover" alt="">
    </button>
    <div
        class="fixed z-[1000] {{ $active === false ? 'hidden' : 'flex' }} inset-0 justify-center items-center px-5 bg-black/50">
        <div class="w-full sm:w-[60%] h-[60%] sm:h-[80%] p-[40px] relative rounded-[8px] bg-white">
            ...
            <div class="w-full h-full overflow-auto">
                <img src="{{ asset('storage/' . $galleryObject->image) }}" class="max-w-none" alt="">
            </div>
        </div>
    </div>
